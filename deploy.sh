...
install_dependencies() {
    ...
    for command in bash curl jq openssl systemctl; do
...
prepare_state() {
    local duration
    install -d -m 700 "$STATE_DIR" "$SOURCES_DIR" "$BACKUP_ROOT"
...
EOF
    chmod 644 /etc/systemd/system/telebotgen.service
...
restore_backup() {
    ...
    rm -rf "$SOURCES_DIR"
    [[ -d "$backup/sources" ]] && cp -a "$backup/sources" "$SOURCES_DIR" || install -d -m 700 "$SOURCES_DIR"
    for mapping in \
        'BotGen.sh:state-BotGen.sh' \
        'vercion:state-vercion' \
        '/bin/ShellBot.sh:bin-ShellBot.sh' \
        '/usr/local/bin/telebotgen-update:bin-telebotgen-update' \
        '/usr/local/bin/telebotgen-deploy:bin-telebotgen-deploy' \
        '/etc/systemd/system/telebotgen.service:service'; do
        destination="${mapping%%:*}"
        source_name="${mapping#*:}"
        [[ "$destination" == /* ]] || destination="$STATE_DIR/$destination"
        if [[ -f "$backup/$source_name" ]]; then
            install -m 700 "$backup/$source_name" "$destination"
        else
            rm -f "$destination"
        fi
    done
    [[ -f /etc/systemd/system/telebotgen.service ]] && chmod 644 /etc/systemd/system/telebotgen.service
    systemctl daemon-reload
...
main() {
    ...
    if ! {
        ...
        install -m 700 "$staging/ShellBot.sh" /bin/ShellBot.sh.new
